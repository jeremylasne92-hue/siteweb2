...
from models import StudentApplication, StudentApplicationRequest, StudentStatusUpdate, StudentBatchStatusUpdate, StudentEditUpdate, User
...
async def submit_student_application(
    ...
    """Submit a student/intern application (public, anti-spam protected)"""
    ...
    try:
        await db.student_applications.insert_one(application.model_dump())
    ...
    logger.info(f"New student application from {data.name}")
    ...
    email_body += f"\nÉcole: {data.school}"
    email_body += f"\nFormation: {data.study_field}"
    email_body += f"\nCompétences: {', '.join(data.skills)}"
    email_body += f"\nDisponibilité: {AVAILABILITY_LABELS.get(data.availability, data.availability)}"
    if data.start_date:
        email_body += f"\nDate début souhaitée: {data.start_date}"
    if data.message:
        email_body += f"\n\nMessage:\n{data.message}"
    background_tasks.add_task(
        send_email,
        ALERT_EMAIL,
        "Nouvelle candidature stage/alternance — Mouvement ECHO",
        email_body,
    )
    background_tasks.add_task(send_student_confirmation, data.email, data.name)
...
async def list_student_applications(
    ...
    """List all student applications (admin only), optionally filtered by status, city, and availability."""
    query = {}
    if status in ("pending", "entretien", "accepted", "rejected"):
        query["status"] = status
    if city != "all":
        query["city"] = city
    if availability in ("stage_court", "stage_long", "alternance", "temps_partiel"):
        query["availability"] = availability

    cursor = db.student_applications.find(query).sort("created_at", -1)
...
async def delete_student_application(
    ...
    """Delete a student application (admin only)."""
    result = await db.student_applications.delete_one({"id": application_id})
    if result.deleted_count == 0:
        raise HTTPException(status_code=404, detail="Candidature non trouvée")
    await log_admin_action(db, admin.id, "delete", "student", application_id)
    logger.info(f"Admin {admin.id} deleted student application {application_id}")
...
async def export_student_applications(
    ...
    """Export all student applications as CSV (admin only)."""
    cursor = db.student_applications.find().sort("created_at", -1)
    applications = []
    ...
    for a in applications:
        ...
        if isinstance(skills, list):
            skills = ", ".join(skills)
        availability_raw = a.get("availability", "")
        availability_label = AVAILABILITY_LABELS.get(availability_raw, availability_raw)
        writer.writerow([
    ...
    output.seek(0)
    logger.info(f"Admin {admin.id} exported {len(applications)} student application records")
...
async def edit_student_application(
    ...
    """Edit student application fields (admin only)."""
    ...
    if result.matched_count == 0:
        raise HTTPException(status_code=404, detail="Candidature non trouvée")
    logger.info(f"Admin {admin.id} edited student application {application_id}")
...
async def update_student_status(
    ...
    """Update a student application status (admin only)."""
    ...
    if result.matched_count == 0:
        raise HTTPException(status_code=404, detail="Candidature non trouvée")
    await log_admin_action(db, admin.id, "status_change", "student", application_id, {"new_status": data.status})
    logger.info(f"Admin {admin.id} updated student application {application_id} to {data.status}")

    # Send status notification email
    application = await db.student_applications.find_one({"id": application_id})
    if application:
        a_email = application["email"]
        a_name = application["name"]
        if data.status == "entretien":
            background_tasks.add_task(send_student_interview, a_email, a_name, settings.BOOKING_URL)
        elif data.status == "accepted":
            background_tasks.add_task(send_student_accepted, a_email, a_name)
        elif data.status == "rejected":
            background_tasks.add_task(send_student_rejected, a_email, a_name, data.status_note)

        # Auto-seed member profile on acceptance
        if data.status == "accepted":
            try:
                await auto_seed_member_profile(db, application, "student")
            except Exception as e:
                logger.error(f"Auto-seed failed for student {application_id}: {e}")
        ...
        if data.status == "rejected":
            try:
                ...
                logger.error(f"Deactivate profile failed for student {application_id}: {e}")
...
async def batch_update_student_status(
    ...
    """Batch update student application statuses (admin only)."""
    update_fields = {"status": data.status, "updated_at": datetime.now(UTC)}
    if data.status_note is not None:
        update_fields["status_note"] = data.status_note
    result = await db.student_applications.update_many(
        {"id": {"$in": data.ids}},
        {"$set": update_fields},
    )
    await log_admin_action(db, admin.id, "batch_status", "student", ",".join(data.ids), {"new_status": data.status})
    logger.info(f"Admin {admin.id} batch-updated {result.modified_count} student applications to {data.status}")
...
async def get_my_student_applications(
    ...
    """Get student applications for the current authenticated user (matched by email)."""
