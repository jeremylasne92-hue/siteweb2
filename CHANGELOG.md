...
- **Backend (Maintenance HOTFIX)** : Suppression des anciens outils de Mock HTTP (`pytest-recording` et `vcrpy`) de l'arborescence et mise à niveau de sécurité de `urllib3` (version 2.6.3+). Les test intègrent désormais nativement FastApi avec la commande basique `pytest tests/`.
